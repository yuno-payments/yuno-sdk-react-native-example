...
export function useVtexWalletCheckout() {
  ...
  /**
   * Step 3 (alternative): present the wallet directly via startPaymentLite with the
   * platform wallet pre-selected — no dependency on the payment-methods list selection
   * event (which is not delivered on iOS bridgeless; see YunoPaymentMethodsViewManager).
   */
  ...
  useYunoEvents({
    ...
    onPaymentStatus: (status: YunoPaymentState) => {
      ...
      // The deferred phase is terminal for this flow — a late status event must not clear it
      setState(prev => ({
        ...prev,
        phase: prev.phase === 'deferred' ? 'deferred' : 'done',
